...
export const postProduct = (product, onSuccess, onError) => async () => {
    try {
        ...
        const errorData = e.response?.data || { message: e.message || "Failed to post product" };
        onError(errorData);
    }
}
...
export const validateCart = (cart, onSuccess, onError) => async (dispatch) => {
    try {
        ...
        const errorData = e.response?.data || { message: e.message || "Cart validation failed" };
        dispatch({ type: VALIDATE_CART_ERR, data: errorData });
        onError(errorData);
    }
}
