...
import { ORDERS_BASEURL, PAYMENTS_BASEURL, PRODUCTS_BASEURL, SHIPPING_BASEURL, USER_BASEURL, CART_BASEURL } from "./BaseURLs";
...
export const authSignup = (formData) => API.post(`${USER_BASEURL}/signup`, formData);
export const verifyOtp = (email, otp) => API.post(`${USER_BASEURL}/verify-otp`, { email, otp });
...
export const fetchCart = () => API.get(`${CART_BASEURL}`);
export const addToCart = (product_id, quantity) => API.post(`${CART_BASEURL}`, { product_id, quantity });
export const removeFromCart = (product_id) => API.delete(`${CART_BASEURL}`, { data: { product_id } });
export const updateCartItem = (product_id, quantity) => API.patch(`${CART_BASEURL}`, { product_id, quantity });
...
export const fetchOrderHistory = () => API.get(`${ORDERS_BASEURL}/history`);
...
export const createOrderCOD = (token, data) => API.post(`${ORDERS_BASEURL}/cod`, { token, data });
