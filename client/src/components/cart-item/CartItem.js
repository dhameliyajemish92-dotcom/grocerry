    ...
    return (
        <div className={styles['wrapper']}>
            ...
            <div className={styles['price']}>
                <div className={styles['total-price']}>{(parseFloat(product.pricing?.selling_price ?? product.price) * parseFloat(product.quantity || '1')).toFixed(2)} ₹</div>
                {parseInt(product.quantity) > 1 && <div
                    className={styles['unit-price']}>{product.pricing?.selling_price ?? product.price} ₹/{product.packaging?.quantity ?? product.weight}{product.packaging?.unit ?? product.measurement}</div>}
            </div>
        </div>
    );
}
